...
// ---------------------------------------------------------------------------
// Sm120 FP8 UMMA path — consumer Blackwell (RTX 6000, cc=12.x)
// CUTLASS v4.4.2 has no Sm120 bf16 UMMA path; only F8F6F4 is supported.
// This path converts bf16 inputs to FP8 E4M3 and runs a fused FP8 GEMM
// with SwiGLU EVT epilogue. The accumulator is float32; the output and
// auxiliary (up) tensors are bf16/f16.
// ---------------------------------------------------------------------------

template <typename ElementOut>
struct FusedSwigluGemmFp8 {
...
// ---------------------------------------------------------------------------
// Generic launch function (same pattern as fused_swiglu_mlp.cu)
// ---------------------------------------------------------------------------
...
template <typename ElementOut>
cutlass::Status run_swiglu_gemm_fp8(
    ...
    using Gemm = typename FusedSwigluGemmFp8<ElementOut>::GemmDevice;
...
// ---------------------------------------------------------------------------
// C-linkage interface for Sm120 FP8 path
//
// Takes FP8 E4M3 inputs (A, B) and bf16/f16 output + aux.
// The host code converts bf16 → FP8 before calling this.
// ---------------------------------------------------------------------------

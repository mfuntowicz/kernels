...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        # Ensure nvcc is discoverable by CMake.  When building inside
        # ``uv pip install`` the PATH may not include the CUDA toolkit.
        nvcc = which("nvcc")
        if nvcc:
            cmake_args.append(f"-DCMAKE_CUDA_COMPILER={nvcc}")
        elif "CUDACXX" not in os.environ and "CMAKE_CUDA_COMPILER" not in os.environ:
            # Try common locations as a last resort
            for candidate in (
                Path("/usr/local/cuda/bin/nvcc"),
                Path("/usr/bin/nvcc"),
            ):
                if candidate.exists():
                    cmake_args.append(f"-DCMAKE_CUDA_COMPILER={candidate}")
                    break
        build_args = []
...
backend = get_backend()
ops_name = f"_fused_swiglu_mlp_{backend}_6cfvnwjfilxus"

setup(
    name="fused_swiglu_mlp",
    # The version is just a stub, it's not used by the final build artefact.
    version="0.1.0",
    ext_modules=[CMakeExtension(f"fused_swiglu_mlp.{ops_name}")],
    cmdclass={"build_ext": CMakeBuild},
    packages=find_packages(where="torch-ext", include=["fused_swiglu_mlp*"]),
